...
@Component({
  selector: 'app-housekeeping-cleaning-stuffing',
  templateUrl: './housekeeping-cleaning-stuffing.component.html',
  styleUrls: ['./housekeeping-cleaning-stuffing.component.scss'],
})
export class HousekeepingCleaningStuffingComponent implements OnInit {
  ...
  onSearch(event, index) {
    // this.housekeepingDetails[index].containerNo = event;
  }
  ...
  onSubmit(form: NgForm) {
    this.open = false;
    if (this.validation()) {
      let requestLineData = this.housekeepingDetails.map(
      ...
      requestLineData.forEach((item) => {
        requestLineList.push({
          ...
          housekeepingRequired: item.housekeeping ? item.housekeeping : false,
          cleaningRequired: item.cleaning ? item.cleaning : false,
          stuffingRequired: item.stuffing ? item.stuffing : false,
          remarks: item.remarks,
        });
      });
      if (!this.isEdit) {
        ...
        let submitData = {
          form: {
            ...
            requestType: 'HOUSEKEEPING_CLEANING_STUFFING',
  ...
  focusOnInvalid() {
    if (this.dateInvalid) {
      ...
      for (let item of this.containerNoRef) {
        if (this.housekeepingDetails[i].invalidContainerNo) {
