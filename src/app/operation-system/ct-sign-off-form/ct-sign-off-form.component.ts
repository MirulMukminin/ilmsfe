...
@Component({
  selector: 'app-ct-sign-off-form',
  templateUrl: './ct-sign-off-form.component.html',
  styleUrls: ['./ct-sign-off-form.component.scss'],
})
export class CtSignOffFormComponent implements OnInit {
  ...
  userInfo() {
    this.appService
      ...
      .then((result) => {
        ...
        this.destinationPort = userInfo?.portOfOrigin;
  ...
  populateData() {
    ...
    this.destinationPort = this.formData?.portOfOrigin;
    ...
    if (this.validation()) {
      ...
      let submitData = {
        requestForm: {
          ...
          portOfOrigin: this.destinationPort,
      ...
      if (!this.isEdit) {
        restServices.pbksb_CrewTransferService
          .createCrewTransferSignOffRequest(this.appService.myApp)(submitData)
          .then((result) => {
            this.isLoading = false;
            this.overlay = false;
        ...
        // Edit request API
        restServices.pbksb_CrewTransferService
          .updateCrewTransferSignOffRequest(this.appService.myApp)(submitData)
          .then((result) => {
            this.isLoading = false;
            this.overlay = false;
            let returnObj = this.appService.jsonToArray(result);
  ...
  downloadExcelTemplate() {
    ...
    link.setAttribute(
      'href',
      './assets/documents/Crew_Arrival_Manifest_Template.csv'
    );
    link.setAttribute('download', `Crew_Arrival_Manifest_Template.csv`);
