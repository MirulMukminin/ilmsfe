...
@Component({
  selector: 'app-ct-sign-on-view',
  templateUrl: './ct-sign-on-view.component.html',
  styleUrls: ['./ct-sign-on-view.component.scss'],
  providers: [TitleCasePipe],
})
export class CtSignOnViewComponent implements OnInit {
  name = '';
  ...
  onCancel() {
    ...
    restServices.pbksb_CrewTransferService
      .cancelCrewTransferSignOnRequest(this.appService.myApp)(params)
      .then((result) => {
  ...
  onEndorse() {
    ...
    restServices.pbksb_CrewTransferService
      .endorseCrewTransferSignOnRequest(this.appService.myApp)(params)
      .then((result) => {
        let returnObj = this.appService.jsonToArray(result);
        let currDate = new Date();
        if (returnObj.success) {
          this.router
            .navigate(['/operation-system/ct-endorsed-sign-on-list'])
            .then(() => {
              this.appService.showToaster({
                type: 'success',
                title: 'Form Endorsed',
