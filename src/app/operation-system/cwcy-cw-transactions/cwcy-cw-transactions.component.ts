...
@Component({
  selector: 'app-cwcy-cw-transactions',
  templateUrl: './cwcy-cw-transactions.component.html',
  styleUrls: ['./cwcy-cw-transactions.component.scss'],
})
export class CwcyCwTransactionsComponent implements OnInit {
  ...
  getRestServicesAPI(company: string) {
    ...
    restServices.pbksb_CommonWarehouseCommonYardService
      .getCommonWarehouseTransactionsByCustomer(this.appService.myApp)({
    ...
    this.model.header = [
      ...
      new TableHeaderItem({ data: 'Weight Released (KG) ' }),
      new TableHeaderItem({ data: 'Weight Remained (KG)' }),
    ];
  }
  ...
  apiRespValidation(value: any) {
    ...
    this.outDate = value.releaseDate
      ...
      : '-';
    this.outQty = !isNaN(value.quantity) ? value.quantity : '-';
    this.initialQty = value.commonWarehouseInventory
      ? value.commonWarehouseInventory.initialQuantity
      : '-';
    this.uom = value.unitOfMeasurement ? value.unitOfMeasurement.name : '-';
    this.weightRelease = !isNaN(value.weight) ? value.weight : '-';
    this.weightRemained = !isNaN(value.actualWeight) ? value.actualWeight : '-';
    /////////////////
  }
