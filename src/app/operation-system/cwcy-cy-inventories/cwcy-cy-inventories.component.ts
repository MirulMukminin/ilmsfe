...
@Component({
  selector: 'app-cwcy-cy-inventories',
  templateUrl: './cwcy-cy-inventories.component.html',
  styleUrls: ['./cwcy-cy-inventories.component.scss'],
})
export class CwcyCyInventoriesComponent implements OnInit {
  ...
  getRestServicesAPI(company: string) {
    ...
    restServices.pbksb_CommonWarehouseCommonYardService
      .getCommonYardInventories(this.appService.myApp)({
    ...
    this.model.header = [
      ...
      new TableHeaderItem({ data: 'Area(m²) Remained' }),
      new TableHeaderItem({ data: 'Initial Area (m²)' }),
  ...
  apiRespValidation(value: any) {
    ...
    if (value.unitOfMeasurement) {
      this.uom = value.unitOfMeasurement.name;
    } else {
      this.uom = '';
    }
    ...
    if (!isNaN(value.area)) {
      this.weightRemained = value.area;
    } else {
      this.weightRemained = null;
    }

    if (!isNaN(value.actualArea)) {
      this.weightInitial = value.actualArea;
    } else {
