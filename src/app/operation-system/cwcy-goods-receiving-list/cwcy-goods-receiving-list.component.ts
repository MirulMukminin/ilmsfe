...
import { ButtonType } from 'carbon-components-angular/button';
import {
  TableHeaderItem,
  TableItem,
  TableModel,
...
@Component({
  selector: 'app-cwcy-goods-receiving-list',
  templateUrl: './cwcy-goods-receiving-list.component.html',
  styleUrls: ['./cwcy-goods-receiving-list.component.scss'],
  providers: [TitleCasePipe],
})
export class CwcyGoodsReceivingListComponent implements OnInit {
  ...
  getRestServicesAPI(company: string) {
    ...
    restServices.pbksb_CommonWarehouseCommonYardService
      .getAllStorageRequestByCustomer(this.appService.myApp)({
  ...
  apiRespValidation(value: any) {
    this.requestNo = value.requestNo ? value.requestNo : 'N/A';
    this.agent = value.agent ? value.agent.name : 'N/A';
    this.location = value.location ? value.location.toUpperCase() : 'N/A';
    this.date_service = value.arrivalDate
      ? formatDate(value.arrivalDate, 'dd/MM/yyyy', 'en_US')
      : 'N/A';
