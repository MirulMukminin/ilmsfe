...
@Component({
  selector: 'app-goods-recieving-endorsed-list',
  templateUrl: './goods-recieving-endorsed-list.component.html',
  styleUrls: ['./goods-recieving-endorsed-list.component.scss'],
  providers: [TitleCasePipe],
})
export class GoodsRecievingEndorsedListComponent implements OnInit {
  ...
  private getRestServiceAPI(customer: string) {
    ...
    //   .getEndorsedGRARequestsByCustomer(this.appService.myApp)(param)
    //   .then((result) => {
  ...
  apiRespValidation(value: any) {
    if (value.requestNo) {
      this.requestNumber = value.requestNo;
    } else {
    ...
    let parsedDate = Date.parse(value.arrivalDate);
    if (isNaN(value.arrivalDate) && !isNaN(parsedDate)) {
      this.theDate = formatDate(value.arrivalDate, 'dd/MM/yyyy', 'en_US');
    } else {
      this.theDate = 'N/A';
    }

    if (isNaN(value.arrivalDate) && !isNaN(parsedDate)) {
      this.theTime = formatDate(value.arrivalDate, 'hh:mm', 'en_US');
    } else {
      this.theTime = 'N/A';
    }
