...
@Component({
  selector: 'app-icw-as-form',
  templateUrl: './icw-as-form.component.html',
  styleUrls: ['./icw-as-form.component.scss'],
})
export class IcwAsFormComponent implements OnInit {
  ...
  serviceTypes = [{ content: 'DECANT' }, { content: 'REPACK' }];
  ...
  */
  ...
  private getRestServiceAPI(customer: string) {
    ...
    restServices.pbksb_ICWAddSRestService
      .getASRequestsByCustomer(this.appService.myApp)(param)
      .then((result) => {
        ...
        for (let i = 0; i < tempLineItem.length; i++) {
          ...
          item.itemDescription = tempLineItem[i].item.itemDescription;
          item.id = tempLineItem[i].item.id;
          item.itemId = tempLineItem[i].item.itemId;
          item.itemCode = tempLineItem[i].item.itemCode;
          //item.outQty = tempLineItem[i].outQty;
          // item.inQty = tempLineItem[i].inQty;
          item.qty = tempLineItem[i].item.qty;
          // item.uom = {name:  tempLineItem[i].item.uom.name}
          item.outUom = tempLineItem[i].outputUOM.name;
          item.outQty = tempLineItem[i].outputQty;
          item.inQty = tempLineItem[i].inputQty;
          item.uom = tempLineItem[i].inputUOM;
          item.transationDate = tempLineItem[i].item.transationDate;
          item.service = tempLineItem[i].serviceType;
          ...
          item.invalidInQty = false;
          item.invalidInQtyText = '';
          item.invalidUom = false;
          item.invalidUomText = '';
          item.invalidService = false;
          item.invalidServiceText = '';
          item.selected = true;
          item.transferToOffshore = false;
    ...
    restServices.pbksb_ICWAddSRestService
      ...
      .then((result) => {
        ...
        for (let i = 0; i < this.typeArray.length; i++) {
          ...
          this.typeArray[i].inQty = 0//this.typeArray[i].qty;
          this.typeArray[i].outUom = this.typeArray[i].chemicalType.uom.name;
          this.typeArray[i].invalidOutQty = false;
          this.typeArray[i].invalidOutQtyText = '';
          this.typeArray[i].invalidInQty = false;
          this.typeArray[i].invalidInQtyText = '';
          this.typeArray[i].invalidUom = false;
          this.typeArray[i].invalidUomText = '';
          this.typeArray[i].invalidService = false;
          this.typeArray[i].invalidServiceText = '';
          this.typeArray[i].transferToOffshore = false;
  ...
  populateData() {
    ...
    } else if (this.detailEditedTransfer.status === 'SUBMITTED') {
      this.isCancelDisabled = false;
      this.isEndorsedDisabled = true;
      this.isEditDisabled = false;
      this.isPrintDisabled = false;
  ...
  onSelectOffShore(event, itemIndex: number) {
    if (event.checked) {
      console.log('itemIndex');
      console.log(itemIndex);
      this.itemToOffshoreChecked.push(itemIndex);
      this.itemList[itemIndex].transferToOffshore = true;
    } else {
      this.itemToOffshoreChecked = this.itemToOffshoreChecked.filter((item) => {
        return item !== itemIndex;
      });
      this.itemList[itemIndex].transferToOffshore = false;
    }
  }
    ...
    if (this.validation()) {
      ...
      if (this.itemlistSelected.length == 0) {
        ...
        if (this.requestType !== 'invalid') {
          ...
          if (!this.isEdit) {
            restServices.pbksb_ICWAddSRestService
              .createASRequestForm(this.appService.myApp)(submitData)
              .then((result) => {
                this.isLoading = false;
                this.overlay = false;
                ...
                if (returnobj.success) {
                  this.router
                    .navigate(['/operation-system/icw-as-list'])
                    .then(() => {
            ...
            console.log(submitData);
            restServices.pbksb_ICWAddSRestService
              .editASRequestForm(this.appService.myApp)(submitData)
              .then((result) => {
                this.isLoading = false;
                this.overlay = false;
                let returnobj = this.appService.jsonToArray(result);
  ...
  populateLineItem() {
    for (let i = 0; i < this.itemlistSelected.length; i++) {
      ...
      console.log('populateLineItem item' + i);
      console.log(item);
      const outOumObj = this.storageLocationSelect.filter(
        (t) => t.content == item.outUom
      );
      //const inOumObj  = this.storageLocationSelect.filter(t => t.content == this.itemList[i].uom.name);
      console.log(outOumObj);

      let populatedItem = {
        _entityName: 'pbksb_ICWASLineItem',
        // outDate: "2022-03-26 18:58:30.000",
        chemicalType: item.chemicalType,
        outputQty: parseInt(item.outQty),
        inputQty: parseInt(item.inQty),
        inputUOM: item.uom,
        outputUOM: {
          id: outOumObj[0].obj.id,
          name: outOumObj[0].obj.name,
        },
        ...
        transferToOffshore: item.transferToOffshore,
      };
  ...
  validation() {
    ...
    for (let i = 0; i < itemList.length; i++) {
      ...
      itemList[i].invalidInQty = false;
      itemList[i].invalidInQtyText = '';
      this.typeArray[i].invalidUom = false;
      this.typeArray[i].invalidUomText = '';
      this.typeArray[i].invalidService = false;
      this.typeArray[i].invalidServiceText = '';
    ...
    for (let i = 0; i < tempList.length; i++) {
      if (tempList[i].selected == true) {
        ...
        /////// in qty validation  //////
        let inQty = tempList[i].inQty;
        // if (inQty) {
        //   inQty = parseInt(inQty, 10);
        //   if (isNaN(inQty)) {
        //     text = 'Input integer value.';
        //     tempList[i].invalidOnQty = true;
        //     tempList[i].invalidInQtyText = text;
        //     this.invaliditemlistText.push({
        //       rowNum: i,
        //       text: text,
        //     });
        //   } else {
        //     if (inQty < 0 || inQty > qty) {
        //       text = 'Input value in range 0:' + qty;
        //       tempList[i].invalidInQty = true;
        //       tempList[i].invalidInQtyText = text;
        //       this.invaliditemlistText.push({
        //         rowNum: i,
        //         text: text,
        ...
        //   });
        // }
        ///////end in qty validation /////

        let outUom = tempList[i].outUom;
        if (!outUom) {
          text = 'Out UOM cannot be empty';
          tempList[i].invalidUom = true;
          tempList[i].invalidUomText = text;
          this.invaliditemlistText.push({
            rowNum: i,
            text: text,
          });
        }
        let service = tempList[i].service;
        if (!service) {
          text = 'Service cannot be empty';
          tempList[i].invalidService = true;
          tempList[i].invalidServiceText = text;
          this.invaliditemlistText.push({
            rowNum: i,
            text: text,
          });
    ...
    for (let i = 0; i < this.itemList.length; i++) {
      if (this.itemList[i].selected) {
        if (this.itemList[i].invalidOutQty) {
          this.focusTableData(i, 3);
          validate = false;
        }
        if (this.itemList[i].invalidInQty) {
          this.focusTableData(i, 2);
          validate = false;
        }
        if (this.itemList[i].invalidUom) {
          this.focusTableData(i, 5);
          validate = false;
        }
        if (this.itemList[i].invalidService) {
          this.focusTableData(i, 7);
          validate = false;
        }
      }
    }
  ...
  inputValueChange(event) {
    ...
    for (let i = 0; i < itemList.length; i++) {
      ...
      itemList[i].invalidOutQty = false;
      itemList[i].invalidOutQtyText = '';
      itemList[i].invalidUom = false;
      itemList[i].invalidUomText = '';
      itemList[i].invalidService = false;
      itemList[i].invalidServiceText = '';
      //}
    }
    this.itemList = itemList;
  ...
  getUOMList() {
    restServices.pbksb_ICWRestService
      ...
      .then((result) => {
        ...
        console.log(this.storageLocationList);
  ...
  getAgents() {
    restServices.pbksb_AgentService
