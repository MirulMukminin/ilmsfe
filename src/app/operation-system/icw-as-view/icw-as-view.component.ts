...
@Component({
  selector: 'app-icw-as-view',
  templateUrl: './icw-as-view.component.html',
  styleUrls: ['./icw-as-view.component.scss'],
  providers: [TitleCasePipe],
})
export class IcwAsViewComponent implements OnInit {
  open = false;
  openEndorsed = false;
  ...
  private getRestServiceAPI(customer: string) {
    ...
    restServices.pbksb_ICWAddSRestService
      ...
      .then((result) => {
        ...
        this.status = this.detailRequestStorage.status? this.detailRequestStorage.status.replaceAll('_', ' '):'';
  ...
  */
  ...
  populateData() {
    ...
    console.log(this.status);
    if (this.detailRequestStorage.status === 'SUBMITTED') {
