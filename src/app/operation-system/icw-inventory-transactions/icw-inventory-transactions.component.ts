...
@Component({
  selector: 'app-icw-inventory-transactions',
  templateUrl: './icw-inventory-transactions.component.html',
  styleUrls: ['./icw-inventory-transactions.component.scss']
})
export class IcwInventoryTransactionsComponent implements OnInit {
  ...
  private getRestServiceAPI(customer: string) {
    ...
    restServices.pbksb_ICWTransferRestService
      .getCustomerInventoryTransactions(this.appService.myApp)(param)
      .then((result) => {
  ...
  apiRespValidation(value: any) {
    ...
    if (value.item.itemType) {
      this.itemType = value.item.itemType;
    } else {
      this.itemType = null;
    }
    */

    if (value.item.itemDescription) {
      this.itemDescription = value.item.itemDescription;
    } else {
      this.itemDescription = null;
    }

    if (value.outQty) {
      this.outQty = value.outQty;
    } else {
      this.outQty = null;
    }

    if (value.requestTransfer.docketNo) {
      this.requestTransfer = value.requestTransfer.docketNo;
    } else {
      this.requestTransfer = null;
    }
  ...
  pushDataSet(index: any) {
    ...
    this.dataset.push({
      ...
      currentQty: this.outQty,
      originQty: this.outQty,
    });
  }
  ...
  getPage(page: number) {
    ...
    for (
      ...
      fullPage.push([
        ...
        this.dataset[i].requestTransfer,
