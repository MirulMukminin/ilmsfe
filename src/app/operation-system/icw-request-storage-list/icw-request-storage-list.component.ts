...
@Component({
  selector: 'app-icw-request-storage-list',
  templateUrl: './icw-request-storage-list.component.html',
  styleUrls: ['./icw-request-storage-list.component.scss'],
  providers: [TitleCasePipe],
})
export class IcwRequestStorageListComponent implements OnInit {
  ...
  private getRestServiceAPI(customer: string) {
    ...
    restServices.pbksb_ICWRestService
      ...
      .then((result: string) => {
        this.goodsRecievingRequestList = JSON.parse(result);
    ...
    this.model.header = [
      ...
      new CustomHeaderItem({ data: 'Status' }),
    ];
  }
  ...
  sortAscendingJobNo = (a, b) => {
    console.log('a type == ', typeof a[this.key]);
    return a[this.key]
      .toString()
