...
@Component({
  selector: 'app-icys-inventory-items',
  templateUrl: './icys-inventory-items.component.html',
  styleUrls: ['./icys-inventory-items.component.scss']
})
export class IcysInventoryItemsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.model.header = [
      ...
      new TableHeaderItem({ data: 'In Date' }),
  ...
  apiRespValidation(value: any) {
    if (value.requestNo) {
      this.requestNo = value.requestNo.requestNo;
    } else {
      this.requestNo = 'N/A';
    }
