...
@Component({
  selector: 'app-icys-transactions',
  templateUrl: './icys-transactions.component.html',
  styleUrls: ['./icys-transactions.component.scss']
})
export class IcysTransactionsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.model.header = [
      ...
      new CustomHeaderItem({ data: 'Recieve No.' }),
      new CustomHeaderItem({ data: 'Release No.' }),
  ...
  apiRespValidation(value: any) {
    if (value.item) {
      this.requestNo = value.item.requestNo.requestNo;
    } else {
      this.requestNo = 'N/A';
    }
    ...
    if (value.item) {
      this.item_name = value.item.item_name;
    } else {
      this.item_name = null;
    }

    if (value.goodsRelease) {
      this.releaseNo = value.goodsRelease.releaseNo;
    } else {
      this.releaseNo = null;
    }
    ...
    if (value.item) {
      this.originalQty = value.item.inQty;
    } else {
      this.originalQty = null;
    }

    if (value.item) {
      this.currentQTy= value.item.qty;
    } else {
  ...
  getPage(page: number) {
    ...
    for (
      ...
      fullPage.push([
        ...
        this.dataset[i].item_name,
        this.dataset[i].outDate,
        this.dataset[i].qty,
        this.dataset[i].currentQTy,
        this.dataset[i].originalQty,        
