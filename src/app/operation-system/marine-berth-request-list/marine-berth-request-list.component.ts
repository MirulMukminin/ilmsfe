...
@Component({
  selector: 'app-marine-berth-request-list',
  templateUrl: './marine-berth-request-list.component.html',
  styleUrls: ['./marine-berth-request-list.component.scss'],
  providers: [TitleCasePipe],
})
export class MarineBerthRequestListComponent implements OnInit {
  ...
  getRestServicesAPI(company: string) {
    ...
    restServices.pbksb_MarineService
      .GetBerthRequestByCompany(this.appService.myApp)(params)
      .then((result) => {
  ...
  protected prepareData(data: Array<Array<any>>) {
    ...
    data.forEach((dataRow) => {
      ...
      dataRow.forEach((dataElement, index) => {
        if (index == 1) {
          if (dataRow[11].toLowerCase().includes('init')) {
            row.push(
              new TableItem({
  ...
  apiRespValidation(value: any) {
    if (value.request_number) {
      this.RequestNo = value.request_number;
    } else {
      this.RequestNo = 'N/A';
    }
    ...
    if (value.status) {
      ...
      this.Status =
        this.Status == 'SENT' || this.Status == 'VERIFIED'
          ? 'Endorsed'
          : this.titlecasePipe.transform(this.Status);
    } else {
      this.Status = 'N/A';
    }
  }
