...
@Component({
  selector: 'app-sw-additional-service-report-list',
  templateUrl: './sw-additional-service-report-list.component.html',
  styleUrls: ['./sw-additional-service-report-list.component.scss'],
  providers: [TitleCasePipe],
})
export class SwAdditionalServiceReportListComponent implements OnInit {
  ...
  getRestServicesAPI(company: string) {
    ...
    restServices.pbksb_ScheduledWasteService
      .getAddServiceReportList(this.appService.myApp)(apiParam)
      .then((result) => {
        let resArr: any = result;
        this.additionalSOList = JSON.parse(resArr);

        this.additionalSOList.sort((a, b) => (a.formNo < b.formNo ? -1 : 1));

        this.additionalSOList.forEach((value, index) => {
    ...
    this.model.header = [
      ...
      new CustomHeaderItem({ data: 'Inbound Form No.' }),
  ...
  pushDataSet(index: any) {
    this.dataset.push({
      number: index + 1,
      formNo: this.formNo,
      inboundFormNo: this.inboundFormNo,
  ...
  apiRespValidation(value: any) {
    ...
    this.inboundFormNo = value.inboundFormNo ? value.inboundFormNo : 'N/A';
