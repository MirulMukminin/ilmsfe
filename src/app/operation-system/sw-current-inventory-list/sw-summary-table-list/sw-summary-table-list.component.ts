...
@Component({
  selector: 'app-sw-summary-table-list',
  templateUrl: './sw-summary-table-list.component.html',
  styleUrls: ['./sw-summary-table-list.component.scss'],
})
export class SwSummaryTableListComponent implements OnInit {
  previousURL = '';
  summaryTableList = [];
  ...
  getRestQueryAPI(Company: string) {
    ...
    restServices.pbksb_ScheduledWasteService
      ...
      .then((result) => {
        ...
        this.summaryTableList = summaryTableResponse.soReportInventoryItemList;

        this.summaryTableList.sort((a, b) =>
          a.itemCode < b.itemCode ? -1 : 1
        );

        this.summaryTableList.forEach((value, index) => {
    ...
    this.model.header = [
      ...
      new TableHeaderItem({ data: 'Weight (MT)' }),
      new TableHeaderItem({ data: 'Location ID' }),
      new TableHeaderItem({ data: 'Qty. Near Expiry Date' }),
  ...
  getPage(page: number) {
    ...
    for (
      ...
      fullPage.push([
        ...
        this.dataset[i].weight,
        this.dataset[i].locationID,
        this.dataset[i].qtyNearExpiryDate,
  ...
  pushDataSet(index: any) {
    this.dataset.push({
      ...
      qtyNearExpiryDate: this.qtyNearExpiryDate,
  ...
  apiRespValidation(value: any) {
    ...
    if (value.packageWeight) {
      this.weight = value.packageWeight;
    } else {
      this.weight = 'N/A';
    }
    if (value.location) {
    ...
    if (value.quantityNearExpiryDate || value.quantityNearExpiryDate == 0) {
      this.qtyNearExpiryDate = value.quantityNearExpiryDate;
    } else {
      this.qtyNearExpiryDate = 'N/A';
    }
    if (value.expiryDate) {
