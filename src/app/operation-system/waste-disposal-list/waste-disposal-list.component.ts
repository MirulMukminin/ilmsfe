...
@Component({
  ...
  styleUrls: ['./waste-disposal-list.component.scss'],
})
export class WasteDisposalListComponent implements OnInit {
  ...
  private getRestServiceAPI(customer: string) {
    ...
    restServices.pbksb_WasteDisposalService
      .getRequestsByCustomer(this.appService.myApp)(param)
      .then((result) => {
        this.wasteDisposalRequestList = this.appService.jsonToArray(result);
        // default sorting on JobNo from latest
        this.wasteDisposalRequestList.sort((a, b) => {
          return b.jobNo.localeCompare(a.jobNo, 'en', { numeric: true });
        });

        this.wasteDisposalRequestList.forEach((value, index) => {
