...
export class GoodsInComponent implements OnInit {
  ...
  getRestQueryAPI(Company: string) {
    ...
    restServices.pbksb_PSBService
      .GetGoodInListByCustomer(this.appService.myApp)(apiParam)
      .then((result) => {
        let resArr: any = result;
        this.goodsInList = JSON.parse(resArr);
        //default sorting result by date starting from latest
        this.goodsInList.sort((a, b) => {
          return (
            this.getTimeTest(b.request_date) - this.getTimeTest(a.request_date)
          );
        });

        this.goodsInList.forEach((value, index) => {
    ...
    this.model.header = [
      ...
      new TableHeaderItem({ data: 'Form Type', className: 'header3' }),
      new TableHeaderItem({ data: 'Form No.', className: 'header4' }),
      new TableHeaderItem({ data: 'Move Date', className: 'header5' }),
      new TableHeaderItem({ data: 'Location', className: 'header6' }),
      new TableHeaderItem({ data: 'Invoice No.', className: 'header7' }),
      new TableHeaderItem({ data: 'Status', className: 'header13' }),
    ];
  }
  ...
  getPage(page: number) {
    ...
    for (
      ...
      fullPage.push([
        ...
        this.dataset[i].invoiceNo,
        // this.dataset[i].goodsDesc,
        // this.dataset[i].quantity,
        // this.dataset[i].rmValue,
        // this.dataset[i].uom,
